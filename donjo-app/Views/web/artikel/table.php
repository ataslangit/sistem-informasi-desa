            ...
            <div class="ui-layout-north panel">
                <div class="left">
                    <div class="uibutton-group">
                        <a href="<?= site_url("web/form/{$cat}")?>" class="uibutton tipsy south" title="Tambah Data"><span class="fa fa-plus">&nbsp;</span>Tambah <?php if ($kategori) {
        echo $kategori['kategori'];
    } else {
        echo 'Artikel Statis';
    }?> Baru</a>
                        <?php if ($_SESSION['grup'] < 4) {?>
                        <button type="button" title="Hapus Artikel" onclick="deleteAllBox('mainform','<?= site_url("web/delete_all/{$p}/{$o}")?>')" class="uibutton tipsy south"><span class="fa fa-trash-o">&nbsp;</span>Hapus
                            <?php }?>
                    </div>
                </div>
                <?php if ($cat < 999) {?>
                <div class="right">
                    <?php if ($_SESSION['grup'] < 4) {?>
                    <button type="button" title="Hapus Kategori <?= $kategori['kategori']?>" onclick="deleteAllBox('mainform','<?= site_url("web/hapus/{$cat}/{$p}/{$o}")?>')" class="uibutton tipsy south"><span class="fa fa-trash-o">&nbsp;</span>Hapus Kategori <?= $kategori['kategori']?>
                        <?php }?>
                </div>
                <?php }?>
            </div>
            <div class="ui-layout-center" id="maincontent" style="padding: 5px;">
                <div class="table-panel top">
                    ...
                    <div class="right">
                        ...
                        <button type="button" onclick="$('#'+'mainform').attr('action','<?= site_url("web/search/{$cat}")?>');$('#'+'mainform').submit();" class="uibutton tipsy south" title="Cari Data"><span class="fa fa-search">&nbsp;</span>Cari</button>
                    </div>
                </div>
                <table class="list">
                    <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="uibutton-group">
                                    <a href="<?= site_url("web/form/{$cat}/{$p}/{$o}/{$data['id']}")?>" class="uibutton tipsy south" title="Ubah Data"><span class="fa fa-pencil"> Ubah </span></a>
                                    <?php if ($_SESSION['grup'] < 4) {?>
                                    <a href="<?= site_url("web/delete/{$cat}/{$p}/{$o}/{$data['id']}")?>" class="uibutton tipsy south" title="Hapus Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"><span class="fa fa-trash-o"></span></a>
                                    <?php if ($data['enabled'] === '2'):?>
                                    <a href="<?= site_url("web/artikel_lock/{$cat}/{$data['id']}")?>" class="uibutton tipsy south" title="Aktivasi artikel"><span class="fa fa-lock"></span></a>
                                    <?php elseif ($data['enabled'] === '1'): ?>
                                    <a href="<?= site_url("web/artikel_unlock/{$cat}/{$data['id']}")?>" class="uibutton tipsy south" title="Non-aktifkan artikel"><span class="fa fa-unlock"></span></a>
                                    <a href="<?= site_url("web/headline/{$cat}/{$p}/{$o}/{$data['id']}")?>" class="uibutton tipsy south" title="Klik untuk Jadikan Headline"><span class="<?php if ($data['headline'] === 1) {?>fa fa-star-o" title="Headline Saat Ini" <?php } else {?>fa fa-star" <?php }?>target="confirm" message="Jadikan Artikel ini sebagai Headline News?" header="Headline"></span></a>
        ...
        <div class="ui-layout-south panel bottom">
            <div class="left">
                <div class="table-info">
                    <form id="paging" action="<?= site_url("web/pager/{$cat}")?>" method="post">
