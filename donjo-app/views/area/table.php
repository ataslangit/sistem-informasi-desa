...
<div class="left">
		...
		<select name="filter" onchange="formAction('mainform','<?= site_url('plan/area/filter')?>')">
		...
		<select name="polygon" onchange="formAction('mainform','<?= site_url('plan/area/polygon')?>')">
		...
		<select name="subpolygon" onchange="formAction('mainform','<?= site_url('plan/area/subpolygon')?>')">
...
<button type="button" onclick="$('#'+'mainform').attr('action','<?= site_url('plan/area/search')?>');$('#'+'mainform').submit();" class="uibutton tipsy south"title="Cari Data"><span class="ui-icon ui-icon-search">&nbsp;</span>Search</button>
</div>
</div>
<table class="list">
<thead>
...
<td>
<a href="<?= site_url("area/form/{$p}/{$o}/{$data['id']}")?>" class="ui-icons icon-edit tipsy south" title="Edit Data"></a><a href="<?= site_url("area/delete/{$p}/{$o}/{$data['id']}")?>" class="ui-icons icon-remove tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a><?php /*if($data['enabled'] == '2'):?><a href="<?php echo site_url('plan/area/area_lock/'.$data['id'])?>" class="ui-icons icon-lock tipsy south" title="Enable area"></a><?php elseif($data['enabled'] == '1'): ?><a href="<?php echo site_url('plan/area/area_unlock/'.$data['id'])?>" class="ui-icons icon-unlock tipsy south" title="Disable area"></a>*/?><a href="<?= site_url("area/ajax_area_maps/{$p}/{$o}/{$data['id']}")?>" target="ajax-modalz" rel="window" header="area <?= $data['nama']?>" class="ui-icons icon-maps tipsy south" title="area <?= $data['nama']?>"></a>
...
<form id="paging" action="<?= site_url('plan/area')?>" method="post">
