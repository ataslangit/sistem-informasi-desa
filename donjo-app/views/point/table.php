                                ...
                                <tbody>
                                    <?php foreach ($main as $data) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="uibutton-group">
                                                    <a href="<?= site_url("point/form/{$p}/{$o}/{$data['id']}") ?>" class="uibutton icon-edit tipsy south" title="Edit Data"></a>
                                                    <a href="<?= site_url("point/delete/{$p}/{$o}/{$data['id']}") ?>" class="uibutton icon-remove tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a>
                                                    <?php if ($data['enabled'] === '2') : ?>
                                                        <a href="<?= site_url('point/point_lock/' . $data['id']) ?>" class="uibutton icon-lock tipsy south" title="Enable point"></a>
                                                    <?php elseif ($data['enabled'] === '1') : ?>
                                                        <a href="<?= site_url('point/point_unlock/' . $data['id']) ?>" class="uibutton icon-unlock tipsy south" title="Disable point"></a>
                                                        <a href="<?= site_url("point/sub_point/{$data['id']}") ?>" class="uibutton icon-document-table tipsy south" title="Rincian Sub point">Rincian</a>
                                                        <a href="<?= site_url("point/ajax_add_sub_point/{$data['id']}") ?>" target="ajax-modal" rel="window" header="Tambah Sub point <?= $data['nama'] ?>" class="uibutton icon-plus tipsy south" title="Tambah Sub point"></a>
                                                    <?php endif ?>
                                                </div>
                                            </td>
