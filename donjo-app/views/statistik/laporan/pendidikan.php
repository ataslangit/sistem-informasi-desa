                ...
                <tbody>
                    <?php foreach ($main as $data) : ?>
                        <tr>
                            ...
                            <td width="5">
                                <a href="<?= site_url("admin_home/form/{$p}/{$o}/{$data['id']}") ?>" class="ui-icons icon-edit tipsy south" title="Edit Data"></a><?php if ($data['username'] !== 'admin') { ?><a href="<?= site_url("admin_home/delete/{$p}/{$o}/{$data['id']}") ?>" class="ui-icons icon-remove tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a>
                                <?php } ?>
                            </td>
                            <td><?= $data['nama'] ?></td>
                            <td><?= $data['grup'] ?></td>
    ...
    <div class="ui-layout-south panel bottom">
        <div class="left">
            <div class="table-info">
                <?= form_open('admin_home', ['id' => 'paging']) ?>
        ...
        <div class="right">
            ...
            <div class="uibutton-group">
                ...
                <?php for ($i = $paging->start_link; $i <= $paging->end_link; $i++) : ?>
                    <a href="<?= site_url("admin_home/index/{$i}/{$o}") ?>" <?php jecho($p, $i, "class='uibutton special'") ?> class="uibutton"><?= $i ?></a>
                <?php endfor; ?>
            </div>
            <div class="uibutton-group">
                <?php if ($paging->next) : ?>
