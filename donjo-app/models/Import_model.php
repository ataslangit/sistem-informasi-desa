...
class Import_model extends Model
{
    ...
    public function import_siak()
    {
        ...
        if (is_file($vfile_upload_dk)) {
            ...
            for ($i = 0; $i < (is_countable($convert) ? count($convert) : 0); $i++) {
        ...
        if (is_file($vfile_upload_bw)) {
            ...
            for ($i = 0; $i < (is_countable($convert1) ? count($convert1) : 0); $i++) {
                if ($j > 0) {
    ...
    public function import_dasar()
    {
        ...
        if ($filename !== '') {
            ...
            for ($a = 1; $a < (count($penduduk) - 1); $a++) {
                ...
                for ($j = 0; $j < (is_countable($p) ? count($p) : 0); $j++) {
                    $in .= ',"' . $p[$j] . '"';
                }
                $in .= '),';
            }
    ...
    public function ppls_rumahtangga()
    {
        ...
        while ($i < (is_countable($rtm) ? count($rtm) : 0)) {
            $o = $rtm[$i]['id'];
            $q = $rtm[$i]['no_kk'];
