                                ...
                                <tbody>
                                    <?php foreach ($main as $data) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="<?= site_url("line/form/{$p}/{$o}/{$data['id']}") ?>" class="ui-icons icon-edit tipsy south" title="Edit Data"></a><a href="<?= site_url("line/delete/{$p}/{$o}/{$data['id']}") ?>" class="ui-icons icon-remove tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"></a><?php if ($data['enabled'] === '2') : ?><a href="<?= site_url('line/line_lock/' . $data['id']) ?>" class="ui-icons icon-lock tipsy south" title="Enable line"></a><?php elseif ($data['enabled'] === '1') : ?><a href="<?= site_url('line/line_unlock/' . $data['id']) ?>" class="ui-icons icon-unlock tipsy south" title="Disable line"></a><a href="<?= site_url("line/sub_line/{$data['id']}") ?>" class="ui-icons icon-document-table tipsy south" title="Rincian Sub line"></a><a href="<?= site_url("line/ajax_add_sub_line/{$data['id']}") ?>" target="ajax-modal" rel="window" header="Tambah Sub line <?= $data['nama'] ?>" class="ui-icons icon-plus tipsy south" title="Tambah Sub line"></a>
                                                <?php endif ?>
                                            </td>
                        ...
                        <div class="right">
                            ...
                            <div class="uibutton-group">
                                <?php for ($i = $paging->start_link; $i <= $paging->end_link; $i++) : ?>
                                    <a href="<?= site_url("line/index/{$i}/{$o}") ?>" <?php jecho($p, $i, "class='uibutton special'") ?> class="uibutton"><?= $i ?></a>
                                <?php endfor; ?>
                            </div>
                            <div class="uibutton-group">
                                <?php if ($paging->next) : ?>
