                                ...
                                <tbody>
                                    <?php foreach ($submenu as $data) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="uibutton-group">
                                                    <a href="<?= site_url("menu/ajax_add_sub_menu/{$tip}/{$menu}/{$data['id']}") ?>" class="uibutton tipsy south" target="ajax-modalx" rel="window" header="Edit Menu" title="Edit Data"><span class="icon-edit icon-large"> Edit </span></a><a href="<?= site_url("menu/delete_sub_menu/{$tip}/{$menu}/{$data['id']}") ?>" class="uibutton tipsy south" title="Delete Data" target="confirm" message="Apakah Anda Yakin?" header="Hapus Data"><span class="icon-trash icon-large"></span></a><?php if ($data['enabled'] === '2') : ?><a href="<?= site_url("menu/menu_lock_sub_menu/{$tip}/{$menu}/{$data['id']}") ?>" class="uibutton tipsy south" title="Enable menu"><span class="icon-lock icon-large"></span></a><?php elseif ($data['enabled'] === '1') : ?><a href="<?= site_url("menu/menu_unlock_sub_menu/{$tip}/{$menu}/{$data['id']}") ?>" class="uibutton tipsy south" title="Disable menu"><span class="icon-unlock icon-large"></span></a><?php endif; ?>
                                                </div>
                                            </td>
                                            <td><?= $data['nama'] ?></td>
                                            <td><?= $data['aktif'] ?></td>
