...
class Analisis_laporan_penduduk_modelx extends Model
{
    ...
    public function search_penduduk_sql()
    {
        if (isset($_SESSION['cari'])) {
            $cari                = $_SESSION['cari'];
            $kw                  = $this->db->escape_like_str($cari);
    ...
    public function pagingy($p = 1, $o = 0)
    {
    ...
    public function list_graph_tanya($id = '')
    {
    ...
    public function list_graph_jawab($id = '', $id2 = '')
    {
        $sql = "SELECT a.id as idmaster, a.nama as pertanyaan, b.id as idanalisis, b.nama as jawaban,
		ifnull((SELECT COUNT(id_pend) FROM analisis_penduduk WHERE id_master=a.id AND id_sub_analisis=b.id AND tahun='{$_SESSION['tahun']}' AND bulan='1'),0) as jan,
		 ifnull((SELECT COUNT(id_pend) FROM analisis_penduduk WHERE id_master=a.id AND id_sub_analisis=b.id AND tahun='{$_SESSION['tahun']}' AND bulan='2'),0) as feb,
    ...
    public function list_data($lap = 0, $o = 0, $offset = 0, $limit = 500)
    {
    ...
    public function list_data_a($o = 0, $offset = 0, $limit = 500)
    {
        $paging_sql = ' LIMIT ' . $offset . ',' . $limit;
        ...
        while ($i < count($data)) {
            ...
            while ($j < count($data1)) {
                ...
                //$sql2 .= $this->tahun_sql();
                //$sql2 .= $this->bulan_sql();
                $query2                  = $this->db->query($sql2, [$data1[$j]['id'], $data[$i]['id']]);
    ...
    public function list_data_b($o = 0, $offset = 0, $limit = 500)
    {
        $paging_sql = ' LIMIT ' . $offset . ',' . $limit;
    ...
    public function list_pertanyaan_penduduk()
    {
    ...
    public function list_data_detail($stat = 0, $jwb = 0, $thn = '', $bln = '', $jns = 0, $lap = 0, $o = 0, $offset = 0, $limit = 500)
    {
        $paging_sql = ' LIMIT ' . $offset . ',' . $limit;
    ...
    public function get_master_analisis_penduduk($id = 0)
    {
        $sql           = "SELECT * FROM master_analisis_penduduk WHERE id={$id}";
        $query         = $this->db->query($sql);
    ...
    public function get_sub_analisis_penduduk($id = 0)
    {
        $sql           = "SELECT * FROM sub_analisis_penduduk WHERE id={$id}";
        $query         = $this->db->query($sql);
